...
struct CADDAnnoExtendedGRCH37
{
    ...
    int encOCC;
    float encOCCombPVal;
    float encOCDNasePVal;
    float encOCFairePVal;
    float encOCpolIIPVal;
    float encOCctcfPVal;
    float encOCmycPVal;
    float encOCDNaseSig;
    float encOCFaireSig;
    ...
    int rare10000bp;
    int sngl10000bp;
    float dbscSNV_ada_score;
    float dbscSNV_rf_score;
    float rawScore;
    float phred;
}
...
alias CADD_EXTENDED_FIELDS_GRCH37 = 
AliasSeq!(
    "CADD_Type",
    "CADD_Length",
    "CADD_Annotype",
...
alias CADD_EXTENDED_DESCRIPTIONS_GRCH37 =
AliasSeq!(
    "Event type (SNV, DEL, INS)",
    "Number of inserted/deleted bases",
    "CodingTranscript, Intergenic, MotifFeature, NonCodingTranscript, RegulatoryFeature, Transcript",
    ...
    "Primate PhyloP score (excl. human) (default: -0.033)",
    "Mammalian PhyloP score (excl. human) (default: -0.038)",
    "Vertebrate PhyloP score (excl. human) (default: 0.017)",
    "Background selection score (default: 800)",
    "targetscan (default: 0*)",
    "mirSVR-Score (default: 0*)",
    "mirSVR-E (default: 0)",
    ...
    "Gerp element score (default: 0)",
    "Gerp element p-Value (default: 0)",
    "Neutral evolution score defined by GERP++ (default: 1.91)",
    "Rejected Substitution score defined by GERP++ (default: -0.2)",
    ...
    "Number of overlapping predicted TF motifs (default: 0)",
    "Reference minus alternate allele difference in nucleotide frequency within an predicted overlapping motif (default: 0)",
    "Result of genomic segmentation algorithm (default: unknown)",
    "Maximum ENCODE H3K27 acetylation level (default: 0)",
    "Maximum ENCODE H3K4 methylation level (default: 0)",
    "Maximum ENCODE H3K4 trimethylation level (default: 0)",
    "Maximum ENCODE expression value (default: 0)",
    "Maximum of ENCODE Nucleosome position track score (default: 0)",
    "ENCODE open chromatin code (default: 5)",
    ...
    "Peak signal for CTCF evidence of open chromatin (default: 0)",
    "Peak signal for Myc evidence of open chromatin (default: 0)",
    "Grantham score: oAA,nAA (default: 0*)",
...
auto processCADDLine(T)(string[] fields){
    ...
    static foreach(i,name;memberNames){
        static if(is(memberTypes[i] == string)){
            mixin("ret."~name.stringof~"=fields[" ~ i.to!string ~ "];");
        }else{
            mixin("ret."~name.stringof~"=fields[" ~ i.to!string ~ "].parse!"~memberTypes[i].stringof~";");
        }
...
void addHeaderFields(bool GRCH37 = false)(ref VCFWriter writer){
    static if(GRCH37){
        alias memberTypes = AliasSeq!(typeof(CADDAnnoExtendedGRCH37.tupleof));
        static foreach (i,m; memberTypes[4..$])
        {
            mixin("writer.addTag!\"INFO\"(" ~ CADD_EXTENDED_FIELDS_GRCH37[i].stringof ~ ",\"A\"," ~ TypeStrings[VcfTypeIndex!m].stringof ~ "," ~ CADD_EXTENDED_DESCRIPTIONS_GRCH37[i].stringof ~");");    
        }
    }else{
...
void addINFOFields(T)(ref VCFRecord rec, ref T anno){
    ...
    static foreach (i,name; memberNames[4..$])
    {
        ...
        }else static if(is(T == CADDAnnoExtendedGRCH37)){
            mixin("rec.addInfo(" ~ CADD_EXTENDED_FIELDS_GRCH37[i].stringof ~ ",anno." ~ name.stringof ~ ");");
        }else{
